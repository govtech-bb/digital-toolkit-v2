...
Content is published as flat files rendered on `alpha.gov.bb` and does not need a content management system. The technical change is straightforward; the harder part is making sure content is readable, at the right reading age, and consistent with the gov.bb house style.
...
**What to build with:** [The GovTech platform for a content service](/getting-started/the-govtech-platform/#for-a-content-service).
...
**Defining characteristic:** contains logic, but no external calls of any kind. Any data it uses is bundled into the site itself – for example a flat file of key information shipped with the code. As soon as a service reads from an external API or database at runtime, it is a complex service.

**Example:** [Bank holidays](https://alpha.gov.bb/bank-holiday-calendar?year=2026).

**What to build with:** [The GovTech platform for a smart content service](/getting-started/the-govtech-platform/#for-a-smart-content-service).
...
**What to build with:** [The GovTech platform for a simple service](/getting-started/the-govtech-platform/#for-a-simple-service).
...
Complex services are published at a subdomain of `alpha.gov.bb` like any other service, for example `your-service.alpha.gov.bb`. A team can build on any domain during development; once a service is adopted onto the platform, it is given its subdomain.
...
**What to build with:** [The GovTech platform for a complex service](/getting-started/the-govtech-platform/#for-a-complex-service).
...
| Smart content | Client-side only | None | No | Tax calculator, next public holiday |
